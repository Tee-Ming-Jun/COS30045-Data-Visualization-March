...
function showPage(page) {
    const sections = ["home", "tv", "storyboard", "svg", "ex4", "about"];
...
// ── CSV loader ───────────────────────────────────────────────────
function loadCSV() {
    fetch("data/TV_data.csv?v=" + Date.now())
        .then(res => res.text())
        .then(data => {
            const rows = data
                .trim()
...
const createBarChart = (data) => {
    ...
    svg.selectAll("rect")
        ...
        .attr("height", d => height - margin - yScale(d.power))
        .attr("fill", "#4e79a7");
